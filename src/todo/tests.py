...
class TestCreateTodoItem(APITestCase):
    ...
    def test_item_was_created(self):
        self.assertEquals(TodoItem.objects.count(), 1)

    def test_item_has_correct_title(self):
        self.assertEquals(TodoItem.objects.get().title, 'Walk the dog')
...
class TestUpdateTodoItem(APITestCase):
    ...
    def setUp(self):
        ...
        self.assertEquals(TodoItem.objects.get().completed, False)
    ...
    def test_item_was_updated(self):
        self.assertEquals(TodoItem.objects.get().completed, True)
...
class TestDeleteTodoItem(APITestCase):
    ...
    def test_item_was_deleted(self):
        self.assertEquals(TodoItem.objects.count(), 0)
...
class TestDeleteAllItems(APITestCase):
    ...
    def setUp(self):
        ...
        createItem(self.client)
        self.assertEquals(TodoItem.objects.count(), 2)
